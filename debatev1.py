...
def republican_node(state: DebateState):
    ...
    return {
        ...
        "history":history
        # turn_count intentionally NOT incremented here — only democrat closes a full turn
    }
...
def democrat_node(state: DebateState):
    ...
    return {
        ...
        "turn_count": state.turn_count + 1,
          "history":history  # A full turn is republican + democrat
    }
...
def build_graph():
    ...
    graph.add_conditional_edges(
        "republican",
        router,
        {"democrat": "democrat", "republican": "republican", "end": END}
    )

    graph.add_conditional_edges(
        "democrat",
        router,
        {"republican": "republican", "democrat": "democrat", "end": END}
    )
...
# --- Run ---
result = graph.invoke(DebateState(question="Should Lgbtq rights be Legalized?"))